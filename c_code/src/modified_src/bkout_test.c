// DO NOT EDIT! THIS FILE WAS GENERATED FROM breakout/bkout_test.c
...
static inline void add_event(int i, int j, colour_t col)
{
  ...
  // spin1_delay_us(500);
  // spin1_send_mc_packet(spike_key, 0, NO_PAYLOAD);
  log_mini_debug("%u%d%d%u%08x", 8003, i, j, col, spike_key);  /* "%d, %d, %u, %08x"*/
}
...
{
  log_mini_info("%u", 8004);  /* "Initialise: started"*/
  ...
  log_mini_info("%u%08x", 8005, key);  /* "\tKey=%08x"*/

  log_mini_info("%u", 8006);  /* "Initialise: completed successfully"*/
...
{
  log_mini_debug("%u%08x", 8007, key);  /* "Packet received %08x"*/
...
void c_main(void)
{
  ...
  {
    log_mini_error("%u", 8008);  /* "Error in initialisation - exiting!"*/
    rt_error(RTE_SWERR);
    return;
  }
