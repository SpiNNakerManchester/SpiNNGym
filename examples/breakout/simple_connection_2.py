...
def paddle_and_ball_list(width, connections, paddle_height=1):
    separator_index = width * paddle_height
    return connections[:separator_index], connections[separator_index:]


def ball_x_projection(width, height, paddle_height=1, weight=0.1, delay=1):
    projection_list = []

    for i in range(paddle_height * int(width), int(width) * int(height)):
        projection_list.append((i, i % 80, weight, delay))


def heuristic_list(width, height, weight=0.1, delay=1):

    connection_list = []

    for i in range(int(width) * int(height)):
        post_index = (np.floor(i // 40) % 2) + 1
        connection_list.append((i, post_index, weight, delay))
...
print("conn_on size:", len(Connections_on))
print("conn_on:", Connections_on)
print("conn_off size:", len(Connections_off))
print("conn_off:", Connections_off)
...
# print("recieve_pop_size = ", receive_pop_size)
...
# Project recieve_pop back to input_pop?
Connections_receive_to_input = heuristic_list(int(X_RESOLUTION/x_factor1), int(Y_RESOLUTION/y_factor1))
p.Projection(receive_pop, spike_input, p.FromListConnector(Connections_receive_to_input),
             p.StaticSynapse(weight=weight))
